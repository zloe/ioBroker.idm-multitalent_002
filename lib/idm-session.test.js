...
describe('idm-session (connection + request/response state machine)', () => {
    ...
    describe('full settings cycle timing', () => {
        it('logs the elapsed time for one full settings cycle (every settings block once) once the next settings cycle starts', () => {
            const socket = connect();
            // idm701100 has 4 sensor blocks (one sweep each) and 5 settings blocks (one per
            // sweep, round-robin), so a full settings cycle - every settings block once - spans
            // 5 sweeps.
            const stepBlocks = (n) => {
                for (let i = 0; i < n; i++) {
                    clock.tick(500);
            ...
            stepBlocks(4); // sensor 09, 0A, 0B, then settings block 03 - starts the first settings cycle

            expect(log.info.getCalls().some(c => /completed one full settings cycle/.test(c.args[0])),
                'the very first settings cycle has no predecessor to time yet').to.be.false;

            stepBlocks(5); // sweep 2 - settings block 04
            stepBlocks(5); // sweep 3 - settings block 05
            stepBlocks(5); // sweep 4 - settings block 06
            stepBlocks(5); // sweep 5 - settings block 08 - completes the first full settings cycle

            expect(log.info.getCalls().some(c => /completed one full settings cycle/.test(c.args[0])),
                'still only 5 settings blocks in - the cycle wraps on the 6th').to.be.false;

            stepBlocks(5); // sweep 6 - back to settings block 03, the next settings cycle's first block

            const settingsCycleLog = log.info.getCalls().find(c => /completed one full settings cycle/.test(c.args[0]));
            expect(settingsCycleLog, 'expected a log.info call once the next settings cycle starts').to.exist;
            expect(settingsCycleLog.args[0]).to.match(/in 12500ms/);
        });
    });
