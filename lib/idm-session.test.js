...
describe('idm-session (connection + request/response state machine)', () => {
    ...
    describe('adaptive per-data-block content delay (hill-climbs a sweet spot, not "wait forever")', () => {
        ...
        it('shrinks the ease-down step every time it eases a block down, converging on ever finer adjustments (down to a 1ms floor)', () => {
            ...
            // step for next time.
            easeOnce();
            expect(session.decreaseStepByBlock.get('07'), 'step halved after its first use').to.equal(50);
            ...
            for (const expectedStep of expectedSteps) {
                easeOnce();
                expect(session.decreaseStepByBlock.get('07')).to.equal(expectedStep);
            }
        ...
        it('a retry resets a block\'s ease-down step back to the coarse starting point, not just the clean-cycle streak', () => {
            session.currentDataBlock = '07';
            session.contentDelayByBlock.set('07', session.normalDataContentDelay + 10 * session.contentDelayDecreaseStep);
            ...
            expect(session.decreaseStepByBlock.get('07')).to.equal(25);

            session.retryNeededThisCycle = true;
            session.updateContentDelayEstimate();
            expect(session.decreaseStepByBlock.get('07'), 'a retry proved the fine-tuned delay wrong - restart refinement from the coarse step')
                .to.equal(session.contentDelayDecreaseStep);
        });
