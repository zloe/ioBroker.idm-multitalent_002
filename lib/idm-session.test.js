...
describe('idm-session (connection + request/response state machine)', () => {
    ...
    describe('full-coverage cycle timing (every sensor AND settings block actually READ at least once)', () => {
        ...
        it('logs the elapsed time and a running total only once every distinct data block has actually been read, each time the lap completes', () => {
            ...
            // pollPattern alternates sensor/settings 1:1 (see its own comment), so a full-coverage
            // cycle needs exactly 10 turns: 5 sensor turns (easily enough to cycle its 4-block
            // round robin more than once over) and exactly 5 settings turns - one full round trip
            // of its 5-block round robin, which is what actually gates it (sensor's smaller group
            // always finishes its own round robin first and just keeps re-cycling).
            ...
            for (let i = 0; i < 8; i++) completeCurrentBlockAndAdvance(socket); // turns 1-8: settings 03, 04, 05, 06
            expect(fullCoverageLogs(), 'only 4 of 5 settings blocks read so far').to.have.lengthOf(0);

            for (let i = 0; i < 2; i++) completeCurrentBlockAndAdvance(socket); // turns 9-10: settings 08 - the last one
            ...
            // Second lap (10 more turns) - now there's a predecessor, so this one does get logged.
            for (let i = 0; i < 10; i++) completeCurrentBlockAndAdvance(socket);
    ...
    describe('multi-block collection, sensor and settings (S_H726100 - see pollPattern / beginGroupCollection())', () => {
        ...
        it('happy path: one full pattern cycle (sensor, settings), each group completing on its first content reply', () => {
            ...
            // One full pattern cycle is exactly one sensor turn and one settings turn now (1:1
            // alternation - see pollPattern's comment): 4 + 7 = 11, one read of every distinct block.
    ...
    describe('wireLength learning (see IdmSession#learnWireLengthFrom / IdmProtocol#recordMeasuredWireLength)', () => {
        ...
        it('a group that graduates mid-run switches from round-robin to multi-block requests starting with its very next turn', () => {
            ...
            clock.tick(session.requestInitDelay);
            socket.emit('data', versionResponse('idm701100')); // -> turn 2 (settings - pollPattern alternates 1:1) begins

            // idm701100's settings blocks were never touched above, so turn 2 is still the classic
            // single-block path - sensor's own graduation must not affect the OTHER group, and
            // pollPattern's next sensor turn isn't until turn 3.
            expect(session.activeMultiBlockGroup, 'turn 2 (settings) has not graduated - still classic single-block').to.be.null;
            ...
            socket.emit('data', idm.create_message('01F20007' + '00'.repeat(10)));
            clock.tick(session.requestInitDelay);
            socket.emit('data', versionResponse('idm701100')); // -> turn 3 (sensor again) begins

            expect(session.activeMultiBlockGroup, 'turn 3 should go straight to a multi-block collection').to.equal('sensor');
            const writesBefore = socket.written.length;
            clock.tick(session.requestDataBlockDelay);
