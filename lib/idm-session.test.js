...
describe('idm-session (connection + request/response state machine)', () => {
    ...
    describe('full-coverage cycle timing (every sensor AND settings block actually READ at least once)', () => {
        ...
        it('logs the elapsed time and a running total only once every distinct data block has actually been read, each time the lap completes', () => {
            const socket = connect();
            // idm701100 has 4 sensor blocks and 5 settings blocks - 9 distinct block ids total.
            // pollPattern reads sensor twice for every one settings read (see its own comment),
            // so a full-coverage cycle needs exactly 15 turns: 10 sensor turns (easily enough to
            // cycle its 4-block round robin more than twice over) and exactly 5 settings turns -
            // one full round trip of its 5-block round robin, which is what actually gates it.
            const fullCoverageLogs = () => log.info.getCalls().filter(c => /^full-coverage cycle #/.test(c.args[0]));

            socket.emit('data', versionResponse('idm701100')); // sensor block 07 in flight (turn 1)

            for (let i = 0; i < 5; i++) completeCurrentBlockAndAdvance(socket); // turns 1-5: settings 03 (turn 3)
            expect(fullCoverageLogs(), 'only 1 of 5 settings blocks read so far').to.have.lengthOf(0);

            for (let i = 0; i < 7; i++) completeCurrentBlockAndAdvance(socket); // turns 6-12: settings 04, 05, 06
            expect(fullCoverageLogs(), 'only 4 of 5 settings blocks read so far').to.have.lengthOf(0);

            for (let i = 0; i < 3; i++) completeCurrentBlockAndAdvance(socket); // turns 13-15: settings 08 - the last one
            ...
            // Second lap (15 more turns) - now there's a predecessor, so this one does get logged.
            for (let i = 0; i < 15; i++) completeCurrentBlockAndAdvance(socket);
            ...
            // floor (no retries happened in this test), so they all fall into ONE group in the
            // grouped-by-delay summary - listed sensor-blocks-first, matching allDataBlockIds().
            expect(logs[0].args[0]).to.match(/^full-coverage cycle #1 done in \d+ms, delays: \(07,09,0A,0B,03,04,05,06,08\) 650ms$/);
            ...
            expect(logs[1].args[0]).to.match(/^full-coverage cycle #2 done in \d+ms, delays: /);
        });

        it('blockDelaysSummary() reflects a grown per-block delay, not just the default, GROUPED by shared delay value', () => {
            ...
            expect(session.blockDelaysSummary()).to.equal(
                '(07) ' + (session.normalDataContentDelay + session.contentDelayIncreaseStep) + 'ms' +
                ', (09,0A,0B,03,04,05,06,08) ' + session.normalDataContentDelay + 'ms'
            );
        });
    ...
    describe('multi-block collection, sensor and settings, interleaved (S_H726100 - see pollPattern / beginOrContinueGroupCollection())', () => {
        ...
        /** Connects and reports S_H726100 - the very first turn (sensor - see pollPattern) is about to begin. */
        ...
        /**
         * Fully drives exactly ONE turn (whatever group pollPattern selects next) to completion
         * and re-syncs (init/version roundtrip), the same way every turn - single-block or
         * multi-block - ends. The 10s tick is generous on purpose: it comfortably covers a
         * request/ack/content-reply roundtrip plus any scripted 'NR' retries within this turn
         * (each only costs session.retryDataContentDelay, 300ms by default) without ever reaching
         * anywhere near the 8s response watchdog that gets (re-)armed once send_init actually
         * writes the init message at the end of the turn - so it never risks ticking PAST that
         * point and tripping a spurious disconnect before we get to supply the version reply
         * ourselves right after.
         */
        ...
        it('the first turn after connecting requests the full SENSOR group as one batch (sensor comes first in pollPattern)', () => {
        ...
        it('happy path: one full pattern cycle (sensor, sensor, settings), each group completing on its first attempt', () => {
            const socket = connectAsH726100();
            ...
            completeGroupTurn(socket); // turn 2: sensor (a fresh collection - see pollPattern's comment)
            // Turn 3 (settings) is driven WITHOUT the trailing version reply, so the state can be
            // inspected right at the end of this pattern cycle, before turn 4 (the version reply
            ...
            clock.tick(10000); // turn 3: settings

            // Sensor is collected TWICE per pattern cycle (2 sensor turns for every 1 settings
            // turn - each turn starts a brand new collection since the previous one already
            // completed and was deleted) and settings once: 4 + 4 + 7 = 15, not just 11.
            expect(hooks.onDataBlockText.callCount).to.equal(15);
            ...
            expect(session.multiBlockCollectors.size).to.equal(0);
        ...
        it('a partial settings reply leaves only the still-missing blocks in that group\'s NEXT attempt (fresh-missing-only, not a resumed stream)', () => {
            const socket = connectAsH726100();
            new MultiBlockControllerSim(socket, session, { settings: [['03', '04']] });

            completeGroupTurn(socket); // turn 1: sensor
            completeGroupTurn(socket); // turn 2: sensor
            completeGroupTurn(socket); // turn 3: settings - first attempt, only 03+04 come back
            expect(session.multiBlockCollectors.get('settings').missing).to.deep.equal(new Set(['05', '06', '08', '09', '0A']));

            completeGroupTurn(socket); // turn 4: sensor
            completeGroupTurn(socket); // turn 5: sensor

            const writesBefore = socket.written.length;
            clock.tick(session.requestDataBlockDelay); // turn 6: settings - its next attempt

            expect(idm_u.get_string_uint8array(socket.written[writesBefore])).to.equal(
                idm_u.get_string_uint8array(idm.create_request_multi_block_message(['05', '06', '08', '09', '0A']))
            );
        });
        ...
        it('a block repeated in a later reply is not double-reported (the collector ignores blocks it already has)', () => {
            ...
            completeGroupTurn(socket); // sensor
            completeGroupTurn(socket); // sensor
            completeGroupTurn(socket); // settings, attempt 1: 03, 04
            completeGroupTurn(socket); // sensor
            completeGroupTurn(socket); // sensor
            completeGroupTurn(socket); // settings, attempt 2: 03 (repeat, ignored), 05, 06, 08, 09, 0A

            expect(hooks.onDataBlockText.withArgs('Data_block_3').callCount, 'block 03 must only be reported once').to.equal(1);
            expect(session.multiBlockCollectors.has('settings')).to.be.false; // completed and cleared
        });

        it('an exact stale repeat (control answered again too soon) does not lose progress or stall the group', () => {
            const socket = connectAsH726100();
            new MultiBlockControllerSim(socket, session, {
                settings: [
                    ['03', '04'],
            ...
            // 4 settings attempts needed (03+04, REPEAT, REPEAT, 05+06+08+09+0A) -> 4 settings
            // turns -> exactly 4*3 = 12 total turns (settings only gets 1 turn in every 3 - see
            // pollPattern). Driving further would start a 2nd settings lap that (with the script
            // exhausted and clamped on its last entry) never re-finds 03/04 - fine for a REAL
            // control (which wouldn't keep "forgetting" blocks it already sent), but not something
            // this deliberately finite script should be pushed into.
            for (let i = 0; i < 12; i++) completeGroupTurn(socket);

            expect(session.multiBlockCollectors.has('settings')).to.be.false;
            for (const id of ALL_SETTINGS_BLOCKS) {
                expect(hooks.onDataBlockText.calledWith('Data_block_' + Number.parseInt(id, 16))).to.be.true;
            }
        });

        it('"NR" (not ready) before a group has anything ready is handled within the same attempt, like the single-block path', () => {
            const socket = connectAsH726100();
            new MultiBlockControllerSim(socket, session, { settings: ['NR', 'NR', [...ALL_SETTINGS_BLOCKS]] });

            completeGroupTurn(socket); // sensor
            completeGroupTurn(socket); // sensor
            completeGroupTurn(socket); // settings - two NRs then the full reply, all within this one turn/attempt

            expect(session.multiBlockCollectors.has('settings')).to.be.false;
            expect(session.multiBlockCollectors.get('settings')?.attempts ?? 0).to.not.be.greaterThan(1);
            for (const id of ALL_SETTINGS_BLOCKS) {
                expect(hooks.onDataBlockText.calledWith('Data_block_' + Number.parseInt(id, 16))).to.be.true;
            }
        ...
        it('gives up after multiBlockMaxAttemptsPerLap attempts and resumes normally instead of stalling forever', () => {
            const socket = connectAsH726100();
            ...
            // Settings only gets 1 turn in every 3 (see pollPattern) - 20 attempts (see
            // multiBlockMaxAttemptsPerLap) therefore needs 60 total turns. Drive 59 of them
            // normally, then tick through the 60th (without supplying its version reply yet) so
            // the give-up can be observed right after it happens, before the next group's turn
            // (which would reset session.activeMultiBlockGroup back to 'sensor') begins.
            for (let i = 0; i < 59; i++) completeGroupTurn(socket);
            clock.tick(10000); // turn 60 (settings): 20th attempt still only finds block 03 -> gives up

            expect(session.multiBlockCollectors.has('settings'), 'should have given up and cleared the collector').to.be.false;
    ...
    describe('wireLength learning (see IdmSession#learnWireLengthFrom / IdmProtocol#recordMeasuredWireLength)', () => {
        ...
        it('a group that graduates mid-run switches from round-robin to multi-block requests starting with its very next turn', () => {
            const socket = connect();
            ...
            // graduation happened, so it must complete normally, not switch mid-flight.
            clock.tick(session.requestDataBlockDelay);
            socket.emit('data', idm.create_message('01F10000'));
            clock.tick(session.contentDelayForCurrentBlock());
            socket.emit('data', idm.create_message('01F20007' + '00'.repeat(10)));
            clock.tick(session.requestInitDelay);
            socket.emit('data', versionResponse('idm701100')); // -> turn 2 (sensor again) begins
