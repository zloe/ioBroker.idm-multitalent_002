...
describe('idm-session (connection + request/response state machine)', () => {
    ...
    describe('adaptive per-data-block content delay (fewer NR retries over time)', () => {
        const dataBlock07 = '00000000000000000000000000000000000000000B270000000000000000';

        it('learns a longer delay from a cycle that needed an NR retry, and uses it (with margin) next time', () => {
            const socket = connect();
            socket.emit('data', versionResponse('idm701100'));
            clock.tick(session.requestDataBlockDelay);
            socket.emit('data', idm.create_message('01F10000')); // R1 ack for data block 07 (first requested for idm701100)

            expect(session.contentDelayByBlock.has('07'), 'nothing learned yet').to.be.false;

            // No learned delay yet, so the fixed normalDataContentDelay is used - but the control
            ...
            const learnedDelay = session.contentDelayByBlock.get('07');
            expect(learnedDelay, 'a delay should have been learned for block 07').to.exist;
            expect(learnedDelay, 'should reflect the extra retry wait, not just the base delay').to.be.greaterThan(session.normalDataContentDelay);
            ...
            const expectedDelay = session.contentDelayForCurrentBlock();
            expect(expectedDelay, 'the delay used should now be the learned one, not the fixed default').to.be.greaterThan(session.normalDataContentDelay);

            const writesBefore = socket.written.length;
            clock.tick(session.normalDataContentDelay);
            expect(socket.written.length, 'must not request content yet at the old fixed delay').to.equal(writesBefore);

            clock.tick(expectedDelay - session.normalDataContentDelay);
            expect(socket.written.length, 'should request content once the learned delay has elapsed').to.equal(writesBefore + 1);
            expect(session.protocolState).to.equal(STATE.DATA_CONTENT_REQUESTED);
        ...
        it('contentDelayForCurrentBlock() falls back to normalDataContentDelay for a block nothing has been learned for yet', () => {
        ...
        it('contentDelayForCurrentBlock() never goes below retryDataContentDelay or above 4x the base delay', () => {
            session.currentDataBlock = '07';

            session.contentDelayByBlock.set('07', 1); // absurdly low learned value
            expect(session.contentDelayForCurrentBlock()).to.equal(session.retryDataContentDelay);

            session.contentDelayByBlock.set('07', 100000); // absurdly high learned value
            expect(session.contentDelayForCurrentBlock()).to.equal(session.normalDataContentDelay * 4);
        });

        it('updateContentDelayEstimate() is a no-op without a preceding R1 ack (no dataBlockAckedAt)', () => {
            session.currentDataBlock = '07';
            session.dataBlockAckedAt = null;
            session.updateContentDelayEstimate();
            expect(session.contentDelayByBlock.has('07')).to.be.false;
        });
    });
