...
describe('idm-session (connection + request/response state machine)', () => {
    ...
    describe('full-coverage cycle timing (every sensor AND settings block actually READ at least once)', () => {
        ...
        it('logs the elapsed time and a running total only once every distinct data block has actually been read, each time the lap completes', () => {
            ...
            // All 9 distinct ids have now been read once, completing the very first lap - but like
            // the very first "full poll cycle" line, there's no predecessor to time yet, so this
            // isn't logged until the *next* lap completes.
            ...
            expect(logs[0].args[0]).to.match(/^full-coverage cycle #1 done in \d+ms$/);
            ...
            expect(logs[1].args[0]).to.match(/^full-coverage cycle #2 done in \d+ms$/);
        });
