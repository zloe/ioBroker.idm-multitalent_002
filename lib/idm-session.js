...
class IdmSession {
    ...
    constructor(idm, config, log, hooks) {
        ...
        // block read at least once. Every sensor block is already read on every single sweep, so
        // this is driven entirely by the settings blocks (one per sweep, round-robin) finishing a
        // full lap - see startingNewFullCoverageCycle in request_data().
    ...
    request_data() {
        ...
        // A new full-coverage cycle - every sensor block AND every settings block read at least
        // once - starts once lastSettingsIndex has wrapped back to 0 (detected below, after the
        // modulo, before it's advanced again). Every sensor block is already re-read on every
        // single sweep, so the settings blocks completing one lap is what actually gates this.
        let startingNewFullCoverageCycle = false;
        ...
        if (this.requestingSensorData) {
            ...
            startingNewFullCoverageCycle = this.lastSettingsIndex === 0;
            datablockToRequest = dataBlocks[this.lastSettingsIndex++];
            this.requestingSensorData = true;
        }

        if (startingNewCycle || startingNewFullCoverageCycle) {
            const now = Date.now();
            if (startingNewCycle) {
            ...
            if (startingNewFullCoverageCycle) {
                if (this.fullCoverageCycleStartedAt != null) {
                    this.fullCoverageCycleCount++;
                    // Short on purpose (see request_data_block()'s block-07 line for the same
                    // reasoning) - this only ever logs once a full lap over ALL settings blocks
                    // (and therefore all sensor blocks too, since those are re-read every single
                    // sweep) has completed, never more often than that.
