...
class IdmSession {
    ...
    constructor(idm, config, log, hooks) {
        ...
        this.normalDataContentDelay = 1000; // fallback for a data block we have no learned delay for yet
        ...
        // retry. That time isn't the same for every data block, so instead of a single fixed
        // guess for all of them, this tracks (per data block number) a smoothed estimate of how
        // long it actually took last time, updated in updateContentDelayEstimate() once a block's
        // content is successfully received - see receive_data()'s 'R1' branch for where it's used.
        this.contentDelayByBlock = new Map();
        this.contentDelayEmaAlpha = 0.25; // how strongly a single new measurement shifts the estimate
        this.contentDelayMargin = 1.15; // stay a bit above the smoothed average on purpose
        this.dataBlockAckedAt = null; // when the current data block's "R1" ack was received
    ...
    request_data_block(dataBlock) {
        ...
        if (dataBlock === '07') {
            this.log.info('requesting data block ' + dataBlock + ', total requests ' + this.totalRequests +
                ...
                ', learned content delay ' + (this.contentDelayByBlock.get(dataBlock) ?? this.normalDataContentDelay) + 'ms');
            this.currentRequests = 0;
            this.currentRetries = 0;
        } else {
     ...
     * for its content - the learned estimate for this specific data block (see
     * updateContentDelayEstimate()) if we have one yet, biased slightly upward on purpose so we
     * stay above the smoothed average rather than right on it, otherwise the normalDataContentDelay
     * fallback used for every block before anything has been learned. Bounded so neither a single
     * unusually fast nor a single unusually slow reply can swing the delay to an extreme.
     * @returns {number}
     */
    contentDelayForCurrentBlock() {
        const learned = this.contentDelayByBlock.get(this.currentDataBlock);
        if (learned == null) return this.normalDataContentDelay;
        const withMargin = Math.round(learned * this.contentDelayMargin);
        return Math.max(this.retryDataContentDelay, Math.min(withMargin, this.normalDataContentDelay * 4));
    }

    /**
     * Learns how long the connected control actually took, this time, from acking a data block
     * request to its content being ready - separately per data block (this.currentDataBlock),
     * since some blocks are consistently slower than others. Called once that block's content was
     * successfully received (receive_data()'s 'Data' branch); the elapsed time already includes
     * any "NR" retries and their wait, so a block that needed retries pulls its own estimate up,
     * meaning fewer (ideally zero) retries are needed for it next time - see
     * contentDelayForCurrentBlock(), which is what actually uses the estimate this updates.
     */
    updateContentDelayEstimate() {
        if (this.currentDataBlock == null || this.dataBlockAckedAt == null) return;
        const elapsed = Date.now() - this.dataBlockAckedAt;
        const previous = this.contentDelayByBlock.get(this.currentDataBlock) ?? this.normalDataContentDelay;
        const smoothed = Math.round(this.contentDelayEmaAlpha * elapsed + (1 - this.contentDelayEmaAlpha) * previous);
        this.contentDelayByBlock.set(this.currentDataBlock, smoothed);
        this.dataBlockAckedAt = null;
    }
    ...
    receive_data(data) {
        ...
        if (state == 3) {
            ...
            if (protocolState === 'R1') { // successful data request, must be in state 3 -> 4
                ...
                this.dataBlockAckedAt = Date.now();
