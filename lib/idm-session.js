...
class IdmSession {
    ...
    constructor(idm, config, log, hooks) {
        ...
        this.contentDelayDecreaseStep = 100; // starting ease-down step - see decreaseStepByBlock
        // Per-block ease-down step, starting at contentDelayDecreaseStep and halved (down to a
        // 1ms floor) every time it's actually used to ease a block's delay down - so a block that
        // keeps needing a correction gets coarse 100ms-ish nudges (fast to find the right rough
        // area), while one that's been stable for a while gets progressively finer ones, homing in
        // on its true minimum safe delay instead of only ever landing on multiples of 100ms. Reset
        // back to the coarse starting step whenever a retry proves the current delay too low, so
        // refinement restarts from scratch after any real setback rather than continuing at a tiny
        // step size that already turned out to be wrong.
        this.decreaseStepByBlock = new Map();
     ...
     *
     * The ease-down step itself also isn't fixed - see decreaseStepByBlock: it starts coarse
     * (contentDelayDecreaseStep) and halves every time it's used, down to a 1ms floor, so a block
     * that's been stable for a while gets refined ever more finely instead of only ever landing on
     * multiples of 100ms. A retry resets that block's step back to the coarse starting point,
     * since it just proved the current delay - and by extension the fine-tuning that led there -
     * wrong, so refinement starts over rather than continuing at a step size already shown to be
     * too aggressive.
     */
    updateContentDelayEstimate() {
        ...
        if (this.retryNeededThisCycle) {
            ...
            this.decreaseStepByBlock.set(block, this.contentDelayDecreaseStep);
            return;
        }
        ...
        if (cleanCycles >= this.contentDelayDecayAfterCleanCycles && current > this.normalDataContentDelay) {
            const step = this.decreaseStepByBlock.get(block) ?? this.contentDelayDecreaseStep;
            this.contentDelayByBlock.set(block, Math.max(current - step, this.normalDataContentDelay));
            this.cleanCyclesByBlock.set(block, 0);
            this.decreaseStepByBlock.set(block, Math.max(1, Math.floor(step / 2)));
        } else {
