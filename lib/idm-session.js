...
class IdmSession {
    ...
    constructor(idm, config, log, hooks) {
        ...
        this.settingsCycleStartedAt = null; // when the current full settings cycle (every settings block once) started
    ...
    request_data() {
        ...
        // A new full settings cycle - every settings block once - starts once lastSettingsIndex
        // has wrapped back to 0, detected below (after the modulo, before it's advanced again).
        let startingNewSettingsCycle = false;
        ...
        if (this.requestingSensorData) {
            ...
            startingNewSettingsCycle = this.lastSettingsIndex === 0;
            datablockToRequest = dataBlocks[this.lastSettingsIndex++];
            this.requestingSensorData = true;
        }

        if (startingNewCycle || startingNewSettingsCycle) {
            const now = Date.now();
            if (startingNewCycle) {
            ...
            if (startingNewSettingsCycle) {
                if (this.settingsCycleStartedAt != null) {
                    this.log.info('completed one full settings cycle (every settings block once) in ' + (now - this.settingsCycleStartedAt) + 'ms');
                }
                this.settingsCycleStartedAt = now;
            }
        }
