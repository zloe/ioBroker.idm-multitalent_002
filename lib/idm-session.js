...
class IdmSession {
    ...
    constructor(idm, config, log, hooks) {
        ...
        // whatever is still missing this lap instead - see multiBlockCollectors below.
        ...
        // pollPatternIndex). Before 2.1.0 this was an implicit 1-sensor-block-at-a-time :
        // 1-settings-turn alternation, where a "settings turn" meant "collect the ENTIRE settings
        // batch before doing anything else" - fine for a single-block firmware, but once settings
        // became a multi-block batch (2.0.0) that turn could take many seconds, during which
        // sensor values (the ones people actually watch change) went stale. As of 2.1.0 a
        // multi-block group's "turn" is always just ONE request/reply attempt (see
        // beginOrContinueGroupCollection()), so the pattern below both settles how often each
        // group's turn comes up AND, just by being a pattern instead of a blocking loop, is what
        // stops a slow settings collection from starving sensor updates.
        ...
        // ---- multi-block collection, sensor and/or settings (see beginOrContinueGroupCollection()) ----
        ...
        //
        // group name -> {missing: Set<string>, collected: Map<string,string>, attempts: number} -
        // exists only while a collection lap for that group is in progress, from
        // beginOrContinueGroupCollection() until the lap completes or gives up (see
        // multiBlockMaxAttemptsPerLap) - see finishGroupCollection(), the only place an entry is
        // removed. Unlike the original (2.0.0) design, a group's lap does NOT hold the connection
        // until every block has been found: every turn sends a FRESH multi-block request for
        // whatever is still missing (or, for a lap just starting, everything) and returns to
        // STATE.IDLE as soon as ONE reply - however partial - has been processed, letting the
        // OTHER group's turn(s) run (see pollPattern above) before this group is attempted again.
        // This also sidesteps something we have no real evidence about either way: whether the
        // control can resume a partial multi-block reply after an unrelated request (e.g. a
        // sensor block) was serviced in between. Every attempt here is a fully self-contained
        // 0171(missing)+0172 request/reply pair instead, exactly like the very first attempt.
        this.multiBlockCollectors = new Map();
        ...
        // (non-multi-block) block request instead. Set by beginOrContinueGroupCollection(),
        // read by receive_data() to decide whether an incoming reply is this group's.
        ...
        // just one (per this feature's real captured multi-block traffic).
        this.multiBlockContentDelay = 1000;
        // After this many attempts (turns) without completing a group's lap, give up on it for
        // now (falling back to STATE.IDLE as if the lap had completed) rather than stalling that
        // group forever - its next turn starts a fresh lap for ALL of that group's blocks anyway,
        // so nothing is permanently lost, just deferred.
        this.multiBlockMaxAttemptsPerLap = 20;
    ...
    /**
     * Abandons ALL groups' in-progress multi-block collection laps (see multiBlockCollectors) -
     * called wherever the connection is about to be reset/resynced from scratch, so a group with
     * some blocks already collected this lap doesn't silently keep counting them towards a lap
     * that, from the control's point of view, never happened. The next time each group's turn
     * comes up it starts a completely fresh lap, asking for all of its blocks again - nothing
     * worse than that, exactly like the single-block path already re-asks everything after a
     * reset.
     */
    abandonMultiBlockCollections() {
        this.multiBlockCollectors.clear();
        this.activeMultiBlockGroup = null;
    }
    ...
    /**
     * Starts (if `group` has no lap in progress) or continues (if it does) `group`'s multi-block
     * collection lap - only ever called when idm.firmwareSupportsMultiBlockRequestsForBlocks()
     * is true for all of `group`'s blocks under the connected version (see request_data()).
     * Mirrors request_data_block()'s pacing (the same requestDataBlockDelay pause before actually
     * sending), but always asks for whatever is still missing THIS LAP (everything, for a lap
     * just starting) rather than tracking a single this.currentDataBlock - see
     * multiBlockCollectors' comment for why this never just loops re-asking internally the way
     * the original (2.0.0) design did.
     * @param {'sensor'|'settings'} group
     */
    beginOrContinueGroupCollection(group) {
        let collector = this.multiBlockCollectors.get(group);
        if (!collector) {
            collector = { missing: new Set(this.groupDataBlocks(group)), collected: new Map(), attempts: 0 };
            this.multiBlockCollectors.set(group, collector);
        }
        collector.attempts++;
        this.currentDataBlock = null;
        this.activeMultiBlockGroup = group;
        this.log.debug('requesting ' + group + ' blocks as a batch (attempt ' + collector.attempts + '): ' + [...collector.missing].join(','));
    ...
    /** Sends the 0171 request for the active group's still-missing blocks (see beginOrContinueGroupCollection()). */
    send_multi_block_data_request() {
        ...
        const collector = this.activeMultiBlockGroup ? this.multiBlockCollectors.get(this.activeMultiBlockGroup) : null;
        if (!collector) {
            // The lap was abandoned (e.g. a connection reset) while this send was scheduled -
            // nothing to do.
            this.log.debug('send_multi_block_data_request: no collection in progress, ignore');
            return;
        }
        const requested = [...collector.missing];
    ...
    // idm.firmwareSupportsMultiBlockRequestsForBlocks()) always collects via
    // beginOrContinueGroupCollection(); every other group keeps the plain one-block-per-turn
    // round-robin below, completely unchanged from before 2.1.0.
    request_data() {
        ...
        if (group === 'sensor') {
            ...
            if (this.idm.firmwareSupportsMultiBlockRequestsForSensors(this.version)) {
                this.beginOrContinueGroupCollection('sensor');
                return;
            }
        ...
        if (this.idm.firmwareSupportsMultiBlockRequests(this.version)) {
            this.beginOrContinueGroupCollection('settings');
            return;
        }
    ...
    receive_data(data) {
        ...
        if (state == 3) {
            ...
            if (protocolState.slice(0, 4) === 'Data' && this.activeMultiBlockGroup) {
                ...
                // beginOrContinueGroupCollection()) - bypass interpret_data()/protocol_state()'s
    ...
    /**
     * Handles one 0172 reply for the ACTIVE group's multi-block collection lap (see
     * beginOrContinueGroupCollection() / this.activeMultiBlockGroup). Parses every block the
     * control actually included this time (typically a PARTIAL subset of what was asked for -
     * confirmed real, expected control behavior, not a bug - see
     * beginOrContinueGroupCollection()'s comment), merges newly found blocks into the collector,
     * and ends THIS TURN either way: complete (the lap is done - see finishGroupCollection()) or
     * still missing something (the lap stays open, picked back up next time this group's turn
     * comes round - see pollPattern). Mirrors the single-block 'Data' branch's end-of-cycle
     * bookkeeping (recordBlockRead, going back to STATE.IDLE, scheduling the next send_init)
     * either way.
     * @param {string} received_data
     */
    handleMultiBlockDataReply(received_data) {
        const group = this.activeMultiBlockGroup;
        const collector = group ? this.multiBlockCollectors.get(group) : null;
        if (!collector) {
            // Can't happen (receive_data() only calls this while this.activeMultiBlockGroup is
            // set and has a collector) - guarded anyway so this can't ever throw on a stray/late
            // call.
        ...
        for (const { block, text } of blocks) {
            if (!collector.missing.has(block)) continue; // already had this one this lap (a repeat) - ignore
        ...
        this.log.debug('multi-block reply (' + group + '): found ' + blocks.map((b) => b.block).join(',') +
            ', still missing ' + [...collector.missing].join(',') + ' after ' + collector.attempts + ' attempt(s)');

        if (collector.missing.size === 0) {
            this.log.debug('multi-block collection complete: ' + group);
            this.multiBlockCollectors.delete(group);
            this.finishGroupCollection();
            return;
        }

        if (collector.attempts >= this.multiBlockMaxAttemptsPerLap) {
            this.log.warn('multi-block collection for ' + group + ' gave up after ' + collector.attempts + ' attempt(s), still missing ' +
                [...collector.missing].join(',') + ' - will retry all ' + group + ' blocks from its next turn');
            this.multiBlockCollectors.delete(group); // start completely fresh next time this group's turn comes up
            this.finishGroupCollection();
            return;
        }

        // Still missing something, but under the attempt cap - leave the (unfinished) collector
        // in place and end THIS TURN anyway, exactly like a completed one: the next time this
        // group's turn comes up (see pollPattern), beginOrContinueGroupCollection() picks the
        // SAME collector back up and asks only for what's still missing. This - not an internal
        // re-ask loop - is what stops a slow multi-block collection from starving the OTHER
        // group's turns, unlike the original (2.0.0) design.
        this.finishGroupCollection();
    }

    /**
     * Ends the current turn after a multi-block reply (see handleMultiBlockDataReply(), the only
     * caller) - whether that group's lap just completed, gave up, or is merely pausing until its
     * next turn - and resumes the normal cycle, exactly like the single-block 'Data' branch in
