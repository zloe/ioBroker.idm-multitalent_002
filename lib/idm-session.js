...
class IdmSession {
    ...
    constructor(idm, config, log, hooks) {
        ...
        // Simple 1:1 alternation, sensor first (not just tied, but first) so the very first turn
        // after connecting - and every settings turn - is bracketed by a fresh sensor read rather
        // than opening the connection with the slower-changing settings data. An earlier version
        // gave sensor 2 turns for every 1 settings turn, back when a settings turn could take
        // several times longer than a sensor turn to actually finish (many re-asks against real
        // hardware) - now that a turn always collects its whole group before yielding either way
        // (see above), that imbalance no longer buys sensor anything extra, so plain alternation
        // is both simpler and gives settings its fair share too.
        this.pollPattern = ['sensor', 'settings'];
    ...
    logWireLengthMeasurementStatus() {
        ...
        for (const [group, dataBlocks] of groups) {
            if (!dataBlocks || dataBlocks.length === 0) continue;
