const idm_utils = {
    ...
    get_byte: function (data) {
        ...
        return 16 * idm_utils.get_dec_from_hex(data[0]) + idm_utils.get_dec_from_hex(data[1]);
    },

    get_byte_string: function (data, length) {
        const num = idm_utils.get_byte(data);
    ...
    get_int: function (data) {
        ...
        return idm_utils.get_byte(data) + 256 * idm_utils.get_byte(data.slice(2, 4));
    },

    get_int_string: function (data, length) {
        const num = idm_utils.get_int(data);
        let text = num.toString();
        while (text.length < length) text = '0' + text;
        return text;
    },
