const idm_utils = {
    ...
    get_value_string: function (value, size) {
        // Round first: values coming from create_set_value_message have usually just been
        // divided by a scaling factor (e.g. 0.1), and that division can leave tiny
        // floating-point error (10.1 / 0.1 === 100.99999999999999 in JS), which used to get
        // silently truncated below into the wrong raw value.
        value = Math.round(value);
        // Wrap negative values into the unsigned range this many nibbles can represent
        // (two's-complement style, like get_hex_from_word does for words) - `%` on a negative
        // number in JS returns a negative remainder, which previously produced invalid,
        // non-hex output such as "-1-1-1-1".
        const range = Math.pow(16, size * 2);
