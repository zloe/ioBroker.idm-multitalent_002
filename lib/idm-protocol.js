...
const idm_protocol = {
        ...
        const idm701100_data = {
            ...
            data_blocks: [
                ...
                },
                {
                    block_number: '09',
                    definition: [
                        { field: 'mindrehz2', description: 'min. Drehzhal Ladepumpe', length: 2, factor: 1 },
    ...
    parseProtocol: function (data, definition) {
        ...
        for (let i = 0; i < definition.length; i++) {
            ...
            if (data.length < nextPos) return text + ' **** missing data';
            let valueText = 'ERROR';
            ...
            text = text + entry.field + ':' + valueText + '; ';
            pos = nextPos;
        }
        if (data.length >= nextPos) text = text + ' **** additional data: ' + data.slice(nextPos);
        return text;
    ...
    interpret_data: function (data) {
        ...
        if (block === 9 && data.length >= 26) {
            const definition = idm_protocol.getDefinition(idm_protocol.version, '09');
            if (definition) {
