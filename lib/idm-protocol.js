...
class IdmProtocol {
    constructor() {
        ...
        // getVerifiedBlockWireLength(). Learned passively from real single-block traffic for a
        // block that has no JSON-declared "wireLength" yet, so that firmware EVENTUALLY qualifies
        // for multi-block requests too (see firmwareSupportsMultiBlockRequestsForBlocks())
        // without a maintainer having to hand-verify it against a traffic capture first - see
        // IdmSession's wireLength-learning hook in receive_data().
    ...
    initialize(customDir, logWarn) {
        ...
        // from a previous run (see seedMeasuredWireLength()), so this is a no-op for real
        // operation - but it keeps a test suite that reuses one IdmProtocol instance across
        // several initialize() calls (e.g. one per `it`, via a shared beforeEach) from leaking
        // measurements learned in one test into the next.
     ...
     * tolerate extra trailing bytes the way a single-block reply's own framing already does:
     * either a JSON-declared "wireLength" (hand-verified by a maintainer against a real traffic
     * capture), or a length CONFIRMED from two consecutive live measurements (see
     * recordMeasuredWireLength()/seedMeasuredWireLength()). Deliberately never falls back to the
    ...
    getVerifiedBlockWireLength(version, block) {
        const definitions = this.dataDefinitions.get(version);
        const entry = definitions ? definitions.data_blocks.find((b) => b.block_number === block) : undefined;
        if (entry && typeof entry.wireLength === 'number') return entry.wireLength;
     ...
     * the only caller) for a block that has no JSON-declared "wireLength" yet - a no-op once the
     * block already has a trusted length (see getVerifiedBlockWireLength()), whether from the
     * data blocks file or an earlier confirmed measurement.
     *
     ...
     * different occasions. Two CONSECUTIVE matching measurements are required before a length is
     * trusted enough to unlock multi-block requests for its group (see
     * firmwareSupportsMultiBlockRequestsForBlocks()) - a disagreement simply restarts the count
     * from the new value rather than ever averaging or guessing which one was right. A value
     * restored from a previous run (see seedMeasuredWireLength()) counts as the FIRST of those
     * two measurements, not a free pass - every restart re-confirms once before its group is
     * trusted for multi-block again, exactly like a block seen for the very first time.
     * @param {string | undefined} version
     * @param {string} block
    ...
    /**
     * Seeds `length` as a CANDIDATE measured wireLength for `block`/`version`, restored from a
     * PREVIOUS adapter run (see main.js, which persists confirmed measurements to an ioBroker
     * state and replays them here via this method on startup, before the connection is started).
     *
     ...
     * have no hard evidence a control's wire length is fixed forever rather than depending on
     * some heat pump setting, so treating a restored value as an unconfirmed candidate - needing
     * exactly one fresh matching measurement THIS run, same as any other block - costs one extra
     * request per block per restart but means every block, freshly seen or previously learned,
     * is held to the same standard rather than a persisted value being trusted blindly forever.
     * @param {string} version
     * @param {string} block
     * @param {number} length
     */
     ...
     * Currently true for every version whose settings blocks all have a JSON-declared
     * "wireLength" (S_H726100) OR have since been fully learned via recordMeasuredWireLength().
     * @param {string | undefined} version
     * @returns {boolean}
     */
