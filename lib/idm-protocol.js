...
// Maps the version identifier reported by the heat pump (used as the lookup key throughout
// this module) to the property prefix used in the data blocks file. These differ in case for
// EVR752101/evr752101 - this table is the single place that needs to know that, instead of it
// being repeated (and easy to get wrong, as it previously was for "*_speed") at every lookup.
// A plain shared constant is fine here (unlike the state below) - it is never mutated.
const VERSION_KEYS = [
    ['idm701100', 'idm701100'],
    ['idm712100', 'idm712100'],
    ['idm722100', 'idm722100'],
 ...
 * blocks file" support) and, more importantly, because of received_data_packet/receive_state/
 * receive_chksum/remaining_data: these track the byte-by-byte parse of ONE incoming TCP
 * stream. Two adapter instances talking to two different heat pumps in the same process
 * (e.g. ioBroker "compact mode") would otherwise interleave and corrupt each other's parser
 * state and data block definitions. Each adapter instance must therefore create its own
 * `new IdmProtocol()`.
 */
class IdmProtocol {
    constructor() {
     ...
     * lookup Maps used by the rest of this class.
    ...
    initialize(customFile, logWarn) {
        ...
        for (const [versionKey, dataKey] of VERSION_KEYS) {
            this.dataDefinitions.set(versionKey, data[dataKey + '_data']);
            this.dataBlocks.set(versionKey, data[dataKey]);
            this.sensorDataBlocks.set(versionKey, data[dataKey + '_sensors']);
            this.settingsDataBlocks.set(versionKey, data[dataKey + '_settings']);
            this.speed.set(versionKey, data[dataKey + '_speed']);
        }
    }
