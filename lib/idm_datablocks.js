...
function validateVersionFile(data) {
    ...
    data.data_blocks.forEach((block, blockIndex) => {
        ...
        // Optional: the block's ACTUAL length on the wire, in bytes - see
        // IdmProtocol.getBlockWireLength()'s doc comment for why this can be (and often is)
        // larger than the sum of the fields below. Only present where it has been verified
        // against a real control (currently just S_H726100) - its presence for every settings
        // block is what gates the multi-block request path (see
        // IdmProtocol.firmwareSupportsMultiBlockRequests()).
        if (block.wireLength !== undefined) {
            if (typeof block.wireLength !== 'number' || !Number.isInteger(block.wireLength) || block.wireLength < 0) {
                errors.push(`${blockWhere}: "wireLength", if present, must be a non-negative integer`);
            } else {
                const docSum = block.definition.reduce((s, f) => s + (typeof f.length === 'number' ? f.length : 0), 0);
                if (block.wireLength < docSum) {
                    errors.push(`${blockWhere}: "wireLength" (${block.wireLength}) is smaller than the sum of its documented field lengths (${docSum}) - it can only be equal or larger (undocumented trailing bytes), never smaller`);
                }
            }
        }
