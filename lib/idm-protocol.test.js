...
// idm-protocol.js now exports the class, not a singleton instance, so tests that talked to
// two independent heat pumps could use separate instances - this file only needs one.
const idm_p = new IdmProtocol();
...
describe('idm-protocol', () => {
    ...
    describe('add_to_packet resilience', () => {
        it('gives up on a frame that never terminates instead of growing the buffer forever', () => {
            const p = new IdmProtocol();
            // SOH followed by a few thousand bytes that are never SOH/ETX/EOT - e.g. line
            // noise on the RS422/TCP path, or a serial-to-TCP converter stuck mid-stream.
            const chunk = String.fromCharCode(1) + 'A'.repeat(3000);
            const state = p.add_to_packet(idm_u.stringToUInt8Array(chunk));

            expect(state).to.be.greaterThan(3); // an error code, not "still waiting for more data"
            // the frame was actually abandoned (reset()), not just flagged while still growing
            expect(p.received_data_packet).to.equal('');
            expect(p.receive_state).to.equal(0);
        });

        it('still accepts a real, complete frame right up against a generous size', () => {
            // Sanity check the guard doesn't accidentally clip a real (if unusually large) frame -
            // build one from a normal data block response, which is nowhere near the cap.
            const p2 = new IdmProtocol();
            const message = p2.create_message('01F2000' + '0'.repeat(400));
            const state = p2.add_to_packet(idm_u.stringToUInt8Array(idm_u.get_string_uint8array(message)));
    ...
    describe('initialize with a custom data blocks directory', () => {
        let tmpDir;
        ...
        it('picks up a valid custom file, including custom min/max values', () => {
            ...
            idm_p.initialize(tmpDir, () => { throw new Error('should not warn for a valid file'); });

            const definition = idm_p.getDefinition('idm701100', '03');
            expect(definition[0].min).to.equal(30);
            expect(definition[0].max).to.equal(65);
            expect(idm_p.dataSources.get('idm701100')).to.equal(customFile);
            expect(idm_p.isCustomDefinition('idm701100'), 'overridden version should be reported as custom').to.be.true;
            expect(idm_p.isCustomDefinition('idm712100'), 'a version untouched by the custom directory should still be reported as bundled').to.be.false;
            expect(idm_p.isCustomDefinition('no-such-version'), 'an unknown version is not custom').to.be.false;
        });
        ...
        it('falls back to the bundled defaults and calls logWarn for an invalid custom file', () => {
            ...
            const warnings = [];
            idm_p.initialize(tmpDir, msg => warnings.push(msg));
    ...
    describe('multi-block requests (see IdmSession\'s settings batch collection)', () => {
        // Real payloads captured from a running S_H726100 control (same bytes as the
        // "parsing real captured data blocks" tests above) - their lengths are exactly the
        // "wireLength" values in lib/datablocks/S_H726100.json, verified against real hardware
        // by individually requesting each block and measuring its actual reply length. Block 03
        // in particular has 3 bytes of undocumented trailing data ("* add data: 000000") that a
        // documented-field-length-only approach would miss - this is precisely the class of bug
        // that made multi-block parsing unreliable before wireLength existed.
        const b03 = '00002E001200010F00000001000B2701000100260200000000000000'; // 28 bytes, 3 undocumented
        const b09 = '0000D80E'; // 4 bytes, exactly matches its documented fields
        const b0A = '000000002D00'; // 6 bytes, exactly matches its documented fields

        // S_H726100.json's declared wireLength for every one of its data blocks - see
        // confirmAllS_H726100WireLengths() below.
        const S_H726100_WIRE_LENGTHS = { '03': 28, '04': 87, '05': 30, '06': 23, '07': 36, '08': 36, '09': 4, '0A': 6, '0B': 11, '0C': 42, '0D': 14 };

        /**
         * A JSON-declared "wireLength" is only a SEEDED CANDIDATE now (see
         * getVerifiedBlockWireLength()'s comment), not an instant grant - most of the tests below
         * are about the multi-block REQUEST/REPLY mechanics, not about the seed-then-confirm
         * transition itself (that has its own dedicated tests, in the next describe block), so
         * this confirms every one of idm_p's S_H726100 blocks with a single matching measurement
         * each - exactly what one real confirming read per block would do - to bring it to the
         * same "fully multi-block-capable" state the old, JSON-was-an-instant-bypass design had
         * from the start.
         */
        function confirmAllS_H726100WireLengths() {
            for (const [block, length] of Object.entries(S_H726100_WIRE_LENGTHS)) {
                idm_p.recordMeasuredWireLength('S_H726100', block, length);
            }
        }

        describe('firmwareSupportsMultiBlockRequests / getBlockWireLength', () => {
            it('a JSON-declared wireLength is a seeded candidate, not an instant grant - the group only qualifies once every block is confirmed', () => {
                expect(idm_p.firmwareSupportsMultiBlockRequests('S_H726100'), 'not yet - nothing confirmed').to.be.false;
                confirmAllS_H726100WireLengths();
                expect(idm_p.firmwareSupportsMultiBlockRequests('S_H726100'), 'now - every settings block confirmed').to.be.true;
            });

            it('is true only for S_H726100 - the only firmware with a verified wireLength for every settings block', () => {
                confirmAllS_H726100WireLengths();
                expect(idm_p.firmwareSupportsMultiBlockRequests('S_H726100')).to.be.true;
                for (const version of versions.filter((v) => v !== 'S_H726100')) {
                    expect(idm_p.firmwareSupportsMultiBlockRequests(version), version).to.be.false;
                }
                expect(idm_p.firmwareSupportsMultiBlockRequests('no-such-version')).to.be.false;
                expect(idm_p.firmwareSupportsMultiBlockRequests(undefined)).to.be.false;
            });

            it('getBlockWireLength returns the verified wireLength when present', () => {
                expect(idm_p.getBlockWireLength('S_H726100', '03')).to.equal(28);
                expect(idm_p.getBlockWireLength('S_H726100', '0C')).to.equal(42);
            });

            it('getBlockWireLength falls back to the documented field-length sum otherwise', () => {
                const definition = idm_p.getDefinition('idm701100', '03');
                const docSum = definition.reduce((s, f) => s + f.length, 0);
                expect(idm_p.getBlockWireLength('idm701100', '03')).to.equal(docSum);
            });

            it('getBlockWireLength/firmwareSupportsMultiBlockRequests return null/false for an unknown version or block', () => {
                expect(idm_p.getBlockWireLength('no-such-version', '03')).to.be.null;
                expect(idm_p.getBlockWireLength('S_H726100', 'ZZ')).to.be.null;
            });
        });

        describe('getVerifiedBlockWireLength / recordMeasuredWireLength / seedMeasuredWireLength (wireLength learning)', () => {
            // Fresh instance per describe block (not the shared idm_p) - recordMeasuredWireLength
            // mutates learning state that must not leak into other tests, and there is no public
            // "forget everything measured" method (deliberately - see initialize()'s comment).
            let learning;
            beforeEach(() => {
                learning = new IdmProtocol();
                learning.initialize();
            });

            it('a JSON-declared wireLength is only a seeded candidate after initialize() - needs one fresh confirming measurement, like any other seed', () => {
                // initialize() (in the outer beforeEach) already seeded '03' from S_H726100.json's
                // declared wireLength (28) - but seeding alone is never enough, see
                // getVerifiedBlockWireLength()'s comment.
                expect(learning.getVerifiedBlockWireLength('S_H726100', '03')).to.be.null;
                // one fresh measurement matching the seed is enough to confirm it (same rule as
                // seedMeasuredWireLength() restoring a persisted value - see the dedicated test below).
                expect(learning.recordMeasuredWireLength('S_H726100', '03', 28)).to.deep.equal({ status: 'confirmed', length: 28 });
                expect(learning.getVerifiedBlockWireLength('S_H726100', '03')).to.equal(28);
                // now that it's confirmed, a further call is a no-op - it doesn't touch the stored
                // length (still 28 below) even though it echoes back whatever was passed in (999)
                expect(learning.recordMeasuredWireLength('S_H726100', '03', 999)).to.deep.equal({ status: 'already-verified', length: 999 });
                expect(learning.getVerifiedBlockWireLength('S_H726100', '03')).to.equal(28); // unchanged
            });

            it('returns null for a block with no JSON wireLength and nothing measured yet - never the field-sum fallback', () => {
                expect(learning.getVerifiedBlockWireLength('idm701100', '03')).to.be.null;
                // getBlockWireLength (the OTHER method) still falls back to the field sum - proves
                // getVerifiedBlockWireLength really is a different, stricter check, not an alias
                expect(learning.getBlockWireLength('idm701100', '03')).to.be.a('number').greaterThan(0);
            });

            it('requires two CONSECUTIVE matching measurements before trusting a length', () => {
                expect(learning.recordMeasuredWireLength('idm701100', '03', 20)).to.deep.equal({ status: 'first-measurement', length: 20 });
                expect(learning.getVerifiedBlockWireLength('idm701100', '03')).to.be.null; // not trusted yet

                expect(learning.recordMeasuredWireLength('idm701100', '03', 20)).to.deep.equal({ status: 'confirmed', length: 20 });
                expect(learning.getVerifiedBlockWireLength('idm701100', '03')).to.equal(20);

                // once confirmed, further measurements are a no-op (already-verified), even a
                // disagreeing one - a length only gets to be trusted once, not re-litigated
                expect(learning.recordMeasuredWireLength('idm701100', '03', 99)).to.deep.equal({ status: 'already-verified', length: 99 });
                expect(learning.getVerifiedBlockWireLength('idm701100', '03')).to.equal(20);
            });

            it('a disagreeing measurement restarts the count from the new value instead of averaging or keeping the old one', () => {
                expect(learning.recordMeasuredWireLength('idm701100', '03', 20)).to.deep.equal({ status: 'first-measurement', length: 20 });
                expect(learning.recordMeasuredWireLength('idm701100', '03', 22)).to.deep.equal({ status: 'mismatch', length: 22 });
                expect(learning.getVerifiedBlockWireLength('idm701100', '03')).to.be.null; // still not trusted

                // needs ANOTHER 22 to confirm now, not a 20 - the mismatch reset the baseline
                expect(learning.recordMeasuredWireLength('idm701100', '03', 20)).to.deep.equal({ status: 'mismatch', length: 20 });
                expect(learning.recordMeasuredWireLength('idm701100', '03', 22)).to.deep.equal({ status: 'mismatch', length: 22 });
                expect(learning.recordMeasuredWireLength('idm701100', '03', 22)).to.deep.equal({ status: 'confirmed', length: 22 });
                expect(learning.getVerifiedBlockWireLength('idm701100', '03')).to.equal(22);
            });

            it('measurements for different blocks/versions do not interfere with each other', () => {
                learning.recordMeasuredWireLength('idm701100', '03', 20);
                learning.recordMeasuredWireLength('idm701100', '04', 30);
                learning.recordMeasuredWireLength('idm712100', '03', 20);
                learning.recordMeasuredWireLength('idm701100', '03', 20); // confirms 701100/03 only
                expect(learning.getVerifiedBlockWireLength('idm701100', '03')).to.equal(20);
                expect(learning.getVerifiedBlockWireLength('idm701100', '04')).to.be.null;
                expect(learning.getVerifiedBlockWireLength('idm712100', '03')).to.be.null;
            });

            it('seedMeasuredWireLength restores a CANDIDATE, not a trusted length - one fresh matching measurement is still required', () => {
                learning.seedMeasuredWireLength('idm701100', '03', 41);
                expect(learning.getVerifiedBlockWireLength('idm701100', '03')).to.be.null; // not trusted yet, even though seeded

                // a disagreeing live measurement after a restart is treated exactly like any
                // other mismatch - logged, and the count restarts from the new value
                expect(learning.recordMeasuredWireLength('idm701100', '03', 99)).to.deep.equal({ status: 'mismatch', length: 99 });
                expect(learning.getVerifiedBlockWireLength('idm701100', '03')).to.be.null;
            });

            it('seedMeasuredWireLength + one matching live measurement re-confirms a length with only a single fresh request', () => {
                learning.seedMeasuredWireLength('idm701100', '03', 41);
                expect(learning.recordMeasuredWireLength('idm701100', '03', 41)).to.deep.equal({ status: 'confirmed', length: 41 });
                expect(learning.getVerifiedBlockWireLength('idm701100', '03')).to.equal(41);
            });

            describe('forgetMeasuredWireLengths (see IdmSession#handleMultiBlockDataReply()\'s parse-error branch, the only caller)', () => {
                it('discards a CONFIRMED length, forcing it all the way back through fresh measurement next time', () => {
                    learning.recordMeasuredWireLength('idm701100', '03', 41);
                    learning.recordMeasuredWireLength('idm701100', '03', 41); // confirmed
                    expect(learning.getVerifiedBlockWireLength('idm701100', '03')).to.equal(41);

                    learning.forgetMeasuredWireLengths('idm701100', ['03']);

                    expect(learning.getVerifiedBlockWireLength('idm701100', '03')).to.be.null;
                    // back to square one: a single fresh measurement is not enough by itself anymore -
                    // it needs two consecutive ones again, exactly like a block never seen before
                    expect(learning.recordMeasuredWireLength('idm701100', '03', 41)).to.deep.equal({ status: 'first-measurement', length: 41 });
                    expect(learning.getVerifiedBlockWireLength('idm701100', '03')).to.be.null;
                });

                it('also discards a JSON-declared seed, not just a live measurement - re-seeding from the same value would just recreate the mismatch', () => {
                    // A fresh instance, never confirmed - S_H726100's block '03' is seeded
                    // (unconfirmed) straight from initialize(), untouched by recordMeasuredWireLength().
                    const fresh = new IdmProtocol();
                    fresh.initialize();
                    expect(fresh.firmwareSupportsMultiBlockRequestsForBlocks('S_H726100', ['03'])).to.be.false;

                    fresh.forgetMeasuredWireLengths('S_H726100', ['03']);

                    // Still nothing trusted - and confirming it now takes a fresh MATCHING
                    // measurement, not an instant grant from the (discarded) JSON seed.
                    expect(fresh.getVerifiedBlockWireLength('S_H726100', '03')).to.be.null;
                    expect(fresh.recordMeasuredWireLength('S_H726100', '03', 28)).to.deep.equal({ status: 'first-measurement', length: 28 });
                    expect(fresh.getVerifiedBlockWireLength('S_H726100', '03')).to.be.null;
                });

                it('forgets only the listed blocks, leaving every other block (same or different group) untouched', () => {
                    learning.recordMeasuredWireLength('idm701100', '03', 41);
                    learning.recordMeasuredWireLength('idm701100', '03', 41);
                    learning.recordMeasuredWireLength('idm701100', '04', 20);
                    learning.recordMeasuredWireLength('idm701100', '04', 20);

                    learning.forgetMeasuredWireLengths('idm701100', ['03']);

                    expect(learning.getVerifiedBlockWireLength('idm701100', '03')).to.be.null;
                    expect(learning.getVerifiedBlockWireLength('idm701100', '04'), 'an untouched block must be unaffected').to.equal(20);
                });

                it('is a no-op for an unknown version or an empty/missing block list - never throws', () => {
                    learning.recordMeasuredWireLength('idm701100', '03', 41);
                    learning.recordMeasuredWireLength('idm701100', '03', 41);

                    expect(() => learning.forgetMeasuredWireLengths(undefined, ['03'])).to.not.throw();
                    expect(() => learning.forgetMeasuredWireLengths('no-such-version', ['03'])).to.not.throw();
                    expect(() => learning.forgetMeasuredWireLengths('idm701100', [])).to.not.throw();

                    expect(learning.getVerifiedBlockWireLength('idm701100', '03'), 'none of the above should have touched the real entry').to.equal(41);
                });
            });
        });

        describe('firmwareSupportsMultiBlockRequestsForBlocks / firmwareSupportsMultiBlockRequestsForSensors', () => {
            it('firmwareSupportsMultiBlockRequestsForSensors is true for S_H726100 (all sensor blocks have a JSON wireLength too)', () => {
                expect(idm_p.firmwareSupportsMultiBlockRequestsForSensors('S_H726100')).to.be.true;
                for (const version of versions.filter((v) => v !== 'S_H726100')) {
                    expect(idm_p.firmwareSupportsMultiBlockRequestsForSensors(version), version).to.be.false;
                }
            });

            it('firmwareSupportsMultiBlockRequestsForBlocks is false for an empty or missing block list', () => {
                expect(idm_p.firmwareSupportsMultiBlockRequestsForBlocks('S_H726100', [])).to.be.false;
                expect(idm_p.firmwareSupportsMultiBlockRequestsForBlocks('S_H726100', null)).to.be.false;
                expect(idm_p.firmwareSupportsMultiBlockRequestsForBlocks('S_H726100', undefined)).to.be.false;
            });

            it('the two groups qualify independently - a version with only its settings blocks fully confirmed is not sensor-capable yet', () => {
                const learning = new IdmProtocol();
                learning.initialize();
                const settingsBlocks = learning.getSettingsDataBlocks('idm701100');
                for (const block of settingsBlocks) {
                    learning.recordMeasuredWireLength('idm701100', block, 10); // first measurement
                    learning.recordMeasuredWireLength('idm701100', block, 10); // confirming measurement
                }
                expect(learning.firmwareSupportsMultiBlockRequestsForBlocks('idm701100', settingsBlocks)).to.be.true;
                expect(learning.firmwareSupportsMultiBlockRequestsForSensors('idm701100')).to.be.false; // sensor blocks untouched
            });
        });

        describe('create_request_multi_block_message', () => {
            it('repeats the single-block "<id>00" payload for every requested block, framed and checksummed like any other message', () => {
                const message = idm_p.create_request_multi_block_message(['03', '0A']);
                const payload = idm_u.get_string_uint8array(message.slice(1, message.length - 5));
                const checksumText = idm_u.get_string_uint8array(message.slice(message.length - 4, message.length - 1));
                expect(payload).to.equal('01710300' + '0A00');
                expect(idm_u.read_val(3, checksumText)).to.equal(idm_u.calc_checksum(payload));
                expect(message[0]).to.equal(1); // SOH
                expect(message[message.length - 1]).to.equal(4); // EOT
            });
        });

        describe('parse_multi_block_reply', () => {
            it('parses every block out of a real multi-block reply, in whatever order they arrive', () => {
                const data = '01F200' + '03' + b03 + '09' + b09 + '0A' + b0A;
                const result = idm_p.parse_multi_block_reply('S_H726100', data);
                expect(result.error).to.be.null;
                expect(result.blocks.map((b) => b.block)).to.deep.equal(['03', '09', '0A']);
                // Same text a single-block parseProtocol() call produces for these exact bytes
                // (see the S_H726100 regression snapshots above) - including block 03's 3
                // undocumented trailing bytes, which only a correct wireLength can find.
                expect(result.blocks[0].text).to.equal(idm_p.parseProtocol(b03, idm_p.getDefinition('S_H726100', '03')));
                expect(result.blocks[1].text).to.equal(idm_p.parseProtocol(b09, idm_p.getDefinition('S_H726100', '09')));
                expect(result.blocks[2].text).to.equal(idm_p.parseProtocol(b0A, idm_p.getDefinition('S_H726100', '0A')));
            });

            it('calls setStateFunction for every field of every block, not just the first', () => {
                const data = '01F200' + '09' + b09 + '0A' + b0A;
                const updates = [];
                idm_p.parse_multi_block_reply('S_H726100', data, (name, value) => updates.push([name, value]));
                // block 09 has one real field (min. Drehzahl Ladepumpe), block 0A has one
                // (max. Temp when direct control with external contact) - neither has a
                // statename configured in the bundled definition, so nothing is actually
                // expected to fire; this only guards against an exception being thrown while
                // walking multiple blocks' fields.
                expect(updates).to.be.an('array');
            });

            it('a partial reply (fewer blocks than requested) parses cleanly - the collector decides what is still missing, not the parser', () => {
                const data = '01F200' + '09' + b09;
                const result = idm_p.parse_multi_block_reply('S_H726100', data);
                expect(result.error).to.be.null;
                expect(result.blocks.map((b) => b.block)).to.deep.equal(['09']);
            });

            it('rejects something that is not a multi-block data reply at all', () => {
                expect(idm_p.parse_multi_block_reply('S_H726100', '01F1000000').error).to.match(/not a multi-block data reply/);
                expect(idm_p.parse_multi_block_reply('S_H726100', '01F201').error).to.match(/not a multi-block data reply/);
            });

            it('reports an error (with whatever blocks parsed cleanly before it) on an unknown block id - lost sync', () => {
                const data = '01F200' + '09' + b09 + 'ZZ' + '00';
                const result = idm_p.parse_multi_block_reply('S_H726100', data);
                expect(result.error).to.match(/unknown block id "ZZ"/);
                expect(result.blocks.map((b) => b.block)).to.deep.equal(['09']);
            });

            it('reports an error on a block truncated shorter than its wireLength', () => {
                const data = '01F200' + '03' + b03.slice(0, 10); // block 03 needs 28 bytes, only 5 given
                const result = idm_p.parse_multi_block_reply('S_H726100', data);
                expect(result.error).to.match(/block 03 truncated/);
                expect(result.blocks).to.deep.equal([]);
            });

            it('reports an error on a trailing byte left over after the last complete block', () => {
                const data = '01F200' + '09' + b09 + '0'; // one dangling hex digit, not a full id
                const result = idm_p.parse_multi_block_reply('S_H726100', data);
                expect(result.error).to.match(/trailing byte/);
                expect(result.blocks.map((b) => b.block)).to.deep.equal(['09']);
            });
        });
    });
